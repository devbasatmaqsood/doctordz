...
exports.confirmAppointment = async (req, res) => {
  try {
    ...
    return res.status(200).send({ status: true, message: "Patient successfully check In " });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
exports.completeAppointment = async (req, res) => {
  try {
    ...
    return res.status(200).send({ status: true, message: "Patient successfully check out" });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: false,
...
exports.cancelAppointment = async (req, res) => {
  try {
    ...
    const [walletHistory, uString] = await Promise.all([WalletHistory.findOneAndDelete({ appointment: appointment._id }), UString.findOneAndDelete({ appointment: appointment._id })]);
    ...
    await Promise.all([appointment.save(), userWallet.save(), UString.findOneAndDelete({ appointment: appointment._id }), notification.save()]);
    return res.status(200).send({
      status: true,
