  ...
  try {
    ...
    if (req.query.userId && req.query.userId != null) {
      ...
      if (!user) {
        return res
          .status(200)
          .json({ status: false, message: 'User does not found.', data: [] });
      }
        ...
        adminPromise
          ...
          .then(async (response) => {
            ...
            notification.message =
              'Hey there! A user has just liked your video. Check it out now!';
...
exports.shareCountOfVideo = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message:
        'video has been shared by the user then shareCount has been increased.',
    });
  } catch (error) {
    console.log(error);
    return res
...
exports.createWatchHistory = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: true,
      message:
        'When user view the video then created watchHistory for that video.',
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
