import ContentDialog from "@/component/content/contentDialog";
import RootLayout from "@/component/layout/Layout";
import Button from "@/extra/Button";
import Table from "@/extra/Table";
import { deleteContent, getContent } from "@/store/contentSlice";
...
const Content = () => {
    ...
    const { content } = useSelector((state: RootStore) => state.content)
...

        {
            ...
            Cell: ({ row, index }: { row: any; index: number }) => (
                <div className="userProfile">
                    <img
                        src={row?.icon !== "" ? row?.icon : noImage.src}
                        width="100px"
                        height="100px"
                        alt={`Doctor`}
            ...
            Cell: ({ row }: { row: any }) => (
                <span>
                     <button
                                ...
                                onClick={() => {
                                  dispatch(openDialog({ type: "content", data: row }));
                                }}
                              >
                                <svg
                                  width="26"
    ...
    return (
        <>
        {dialogueType === "content" && <ContentDialog />}
        <div className="mainCategory">
            {
                ...
                <div className="betBox">
                    <Button
                        ...
                        text={"Content"}
                        onClick={() => {
                            dispatch(openDialog({ type: "content" }));
                        }}
                    />
                </div>
            </div>
